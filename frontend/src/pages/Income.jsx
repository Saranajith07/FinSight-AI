...
//small component
const IncomeChart = ({ chartData, timeFrame, timeFrameRange, contextUser }) => (
      ...
      <ResponsiveContainer width="100%" height="100%">
        ...
        >
          ...
          <YAxis
            ...
            tick={{ fill: "#6b7280", fontSize: 12 }}
            width={50}
            tickFormatter={(value) => `${contextUser?.currencySymbol || "$"}${value.toLocaleString()}`}
          />
          <Tooltip
            formatter={(value) => [
              `${contextUser?.currencySymbol || "$"}${Math.round(value).toLocaleString()}`,
              "Income",
            ]}
...
const Income = () => {
  ...
  const handleExport = useCallback(async () => {
    try {
      ...
      let filename = `income_${timeFrame}.xlsx`;
    ...
    <div className={styles.wrapper}>
      ...
      <div className={styles.summaryGrid}>
        <FinancialCard
          ...
          label="Total Income"
          value={`${user?.currencySymbol || "$"}${Number(totalIncome || 0).toLocaleString()}`}
          additionalContent={
        ...
        <FinancialCard
          ...
          value={`${user?.currencySymbol || "$"}${Number(averageIncome || 0).toLocaleString()}`}
          additionalContent={
            <div className="mt-2 text-xs text-gray-500 flex items-center">
