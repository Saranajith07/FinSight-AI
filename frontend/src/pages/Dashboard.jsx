...
// Time-based greeting for a friendlier homepage
const getGreeting = () => {
...
const Dashboard = () => {
  ...
  const handleDownloadReport = async () => {
    try {
      const res = await axios.get(`${API_BASE}/dashboard/download`, {
        headers: getAuthHeader(),
      ...
      const blob = res.data instanceof Blob 
        ? res.data 
        : new Blob([res.data], { type: res.headers["content-type"] || "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" });
      const disposition = res.headers["content-disposition"];
      let filename = `report_${timeFrame}.xlsx`;
    ...
    <div className={dashboardStyles.container}>
      ...
      <div className={dashboardStyles.headerContainer}>
        <div className={dashboardStyles.headerContent}>
          <div>
            <p className="text-sm font-medium text-teal-600 uppercase tracking-wider mb-0.5">
              {getGreeting()}
            </p>
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className="flex items-center gap-2 bg-white border border-slate-200 hover:border-teal-200 hover:bg-teal-50/50 text-slate-700 px-4 py-2.5 rounded-xl transition-all shadow-sm font-medium text-sm"
            >
      ...
      <div className={dashboardStyles.summaryGrid}>
        <FinancialCard
          ...
          label="Total Balance"
          value={`${user?.currencySymbol || "$"}${Math.round(displayIncome - displayExpenses).toLocaleString()}`}
          additionalContent={
            <div className=" flex items-center gap-2 mt-2 text-sm">
              <span className={dashboardStyles.balanceBadge}>
                +{user?.currencySymbol || "$"}{Math.round(displayIncome).toLocaleString()}
              </span>
              <span className={dashboardStyles.expenseBadge}>
                -{user?.currencySymbol || "$"}{Math.round(displayExpenses).toLocaleString()}
              </span>
            </div>
          }
        />
        ...
        <FinancialCard
          ...
          value={`${user?.currencySymbol || "$"}${Math.round(displayIncome).toLocaleString()}`}
          additionalContent={
            <div className="mt-2 text-xs text-green-600 font-medium">
              Tracked from all sources
            </div>
          }
        />
        ...
        <FinancialCard
          ...
          value={`${user?.currencySymbol || "$"}${Math.round(displayExpenses).toLocaleString()}`}
          additionalContent={
            <div
        ...
        <FinancialCard
          ...
          value={`${user?.currencySymbol || "$"}${Math.round(displaySavings).toLocaleString()}`}
          additionalContent={
      ...
      {user && user.budgetLimit > 0 && timeFrame === "monthly" && (
        <div className="bg-white/90 backdrop-blur p-6 rounded-2xl shadow-sm border border-slate-100 mb-6">
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-bold text-gray-800">Monthly Budget Tracker</h3>
                <p className="text-sm text-gray-500">Your limit: {user?.currencySymbol || "$"}{user.budgetLimit.toLocaleString()}</p>
              </div>
            </div>
            <div className="text-right">
              <p className="text-2xl font-bold text-gray-800">
                {Math.round((displayExpenses / user.budgetLimit) * 100)}%
              </p>
              <p className="text-xs text-gray-500 uppercase tracking-wider font-semibold">Spent</p>
            </div>
          </div>
          ...
          <div className="relative h-4 bg-gray-100 rounded-full overflow-hidden">
            <div 
              className={`absolute top-0 left-0 h-full transition-all duration-1000 ease-out ${
                (displayExpenses / user.budgetLimit) >= 1 ? 'bg-red-500' : 
                (displayExpenses / user.budgetLimit) >= 0.8 ? 'bg-orange-500' : 'bg-teal-500'
              }`}
              style={{ width: `${Math.min((displayExpenses / user.budgetLimit) * 100, 100)}%` }}
            ></div>
          </div>
          
          <div className="flex justify-between mt-3 text-sm">
            <span className="font-medium text-gray-600">
              Total Spent: <span className="text-orange-600">{user?.currencySymbol || "$"}{displayExpenses.toLocaleString()}</span>
            </span>
            <span className="font-medium text-gray-600">
              Remaining: <span className="text-teal-600">{user?.currencySymbol || "$"}{Math.max(user.budgetLimit - displayExpenses, 0).toLocaleString()}</span>
            </span>
          </div>
          
          {(displayExpenses / user.budgetLimit) >= 1 && (
      ...
      {/* Category Budgets */}
      {user && user.categoryBudgets && Object.keys(user.categoryBudgets).length > 0 && timeFrame === "monthly" && (
        <div className="bg-white/90 backdrop-blur p-6 rounded-2xl shadow-sm border border-slate-100 mb-6">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h3 className="text-lg font-bold text-gray-800">Category Budgets</h3>
              <p className="text-sm text-gray-500">Tracking specific spending limits</p>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {Object.entries(user.categoryBudgets).map(([cat, limit]) => {
              ...
              const percent = Math.round((catSpent / limit) * 100);
              const isExceeded = catSpent >= limit;
              const isWarning = catSpent >= limit * 0.8;

              return (
                <div key={cat} className="space-y-2">
                  <div className="flex justify-between text-sm">
                    <span className="font-bold text-gray-700">{cat}</span>
                    <span className={`font-semibold ${isExceeded ? 'text-red-500' : isWarning ? 'text-orange-500' : 'text-teal-600'}`}>
                      {user.currencySymbol || "$"}{catSpent.toLocaleString()} / {user.currencySymbol || "$"}{limit.toLocaleString()}
                    </span>
                  </div>
                  <div className="relative h-2 bg-gray-100 rounded-full overflow-hidden">
                    <div 
                      className={`absolute top-0 left-0 h-full transition-all duration-700 ${
                        isExceeded ? 'bg-red-500' : isWarning ? 'bg-orange-500' : 'bg-teal-500'
                      }`}
                  ...
                  <div className="flex justify-between text-[10px] uppercase tracking-wider font-bold text-gray-400">
                    ...
                    <span>{isExceeded ? 'Exceeded' : isWarning ? 'Warning' : 'Healthy'}</span>
                  </div>
                </div>
              );
            })}
        ...
        <div className="bg-white/90 backdrop-blur p-6 rounded-2xl shadow-sm border border-slate-100 mb-6">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">Milestones to your financial success</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {goals.slice(0, 3).map((goal) => {
              ...
              return (
                <div key={goal._id} className="bg-gray-50/50 p-4 rounded-xl border border-gray-100">
                  ...
                  <div className="flex justify-between items-end">
                    <span className="text-xs text-gray-500">
                      Target: {user?.currencySymbol || "$"}{goal.targetAmount.toLocaleString()}
                    </span>
                    <span className="text-sm font-black text-indigo-600">
                      {user?.currencySymbol || "$"}{goal.currentAmount.toLocaleString()}
                    </span>
                  </div>
                </div>
              );
          ...
          {goals.length > 3 && (
            <button 
              ...
              className="mt-6 w-full py-2 text-sm font-bold text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all border border-dashed border-indigo-200"
            >
              View All Goals
            </button>
          )}
        ...
        <div className={dashboardStyles.pieChartHeight}>
          <ResponsiveContainer width="100%" height="100%">
            <PieChart className={chartStyles.pieChart}>
              ...
              <Tooltip
                formatter={(value) => [
                  `${user?.currencySymbol || "$"}${Math.round(value).toLocaleString()}`,
                  "Amount",
                ]}
                ...
                >
                  ...
                  <div className={dashboardStyles.transactionAmount}>
                    <p className={dashboardStyles.incomeAmount}>
                      +{user?.currencySymbol || "$"}{Math.abs(transaction.amount).toLocaleString()}
                    </p>
