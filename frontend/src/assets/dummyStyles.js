...
export const dashboardStyles = {
  // Layout styles - warmer, softer background
  container: "min-h-screen p-4 md:p-6 bg-gradient-to-br from-slate-50 via-teal-50/30 to-cyan-50/40",
  
  // Header styles - cleaner card with subtle gradient
  headerContainer: "bg-white/80 backdrop-blur-md rounded-3xl p-6 md:p-8 mb-8 shadow-sm border border-teal-100/50",
  headerContent: "flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6",
  headerTitle: "text-2xl md:text-3xl font-bold text-slate-800 tracking-tight",
  headerSubtitle: "text-slate-500 mt-1 text-base",
  
  // Button styles
  addButton: "flex items-center gap-2 bg-gradient-to-r from-teal-500 to-cyan-600 hover:from-teal-600 hover:to-cyan-700 text-white px-5 py-3 rounded-xl transition-all shadow-md hover:shadow-lg font-medium",
  
  // Time frame selector styles
  timeFrameContainer: "flex justify-end mt-4",
  timeFrameWrapper: "flex gap-0 bg-slate-100/80 p-1 rounded-xl",
  ...
  // Summary cards grid (4 cards: balance, income, expenses, savings)
  summaryGrid: "grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5 mb-8",
  ...
  pieChartContainer: "hidden md:block bg-white/90 backdrop-blur rounded-2xl p-5 lg:p-6 shadow-sm border border-slate-100 relative overflow-hidden mb-8",
  pieChartHeader: "flex flex-col sm:flex-row justify-between items-start sm:items-center mb-5 gap-3",
  pieChartTitle: "text-xl font-bold text-slate-800 mb-5 flex items-center gap-3",
  pieChartSubtitle: "text-sm text-slate-500 mb-3",
  ...
  listContainer: "bg-white/90 backdrop-blur rounded-2xl lg:p-5 md:p-6 shadow-sm border border-slate-100",
  ...
  listSubtitle: "text-sm text-slate-500 font-normal",
  
  // Record count badges
  incomeCountBadge: "text-sm bg-emerald-100 text-emerald-700 px-2.5 py-1 rounded-full font-medium",
  expenseCountBadge: "text-sm bg-amber-100 text-amber-800 px-2.5 py-1 rounded-full font-medium",
  
  // Transaction item styles
  transactionList: "space-y-3",
  incomeTransactionItem: "flex items-center justify-between p-3 md:p-4 bg-emerald-50/80 rounded-xl border border-emerald-100/50 hover:bg-emerald-50 transition-colors",
  expenseTransactionItem: "flex items-center justify-between p-3 md:p-4 bg-amber-50/80 rounded-xl border border-amber-100/50 hover:bg-amber-50 transition-colors",
...
export const loginStyles = {
  // Page container - background image set inline in component
  pageContainer: "min-h-screen flex items-center justify-center p-4 bg-slate-900 bg-cover bg-center bg-no-repeat",
...
// Modern signup page styles (FinSight AI)
export const signupStyles = {
  // Page: split layout on lg, centered card on mobile. Background image set inline in component.
  pageContainer: "min-h-screen flex items-center justify-center p-4 md:p-6 bg-slate-900 bg-cover bg-center bg-no-repeat",
  
  // Grid: left branding (hidden on small), right form
  grid: "w-full max-w-5xl grid grid-cols-1 lg:grid-cols-2 gap-0 rounded-3xl overflow-hidden shadow-2xl bg-slate-900/50 border border-slate-700/50",
  
  // Left panel - branding (FinSight AI)
  brandPanel: "hidden lg:flex flex-col justify-center p-10 xl:p-14 bg-gradient-to-br from-teal-600 via-cyan-700 to-slate-800 relative overflow-hidden",
  brandOverlay: "absolute inset-0 bg-white/5",
  brandLogo: "text-3xl xl:text-4xl font-bold text-white tracking-tight lobster-regular",
  brandTagline: "text-teal-100/90 mt-4 text-lg max-w-sm leading-relaxed",
  brandFeatures: "mt-8 space-y-3",
  brandFeature: "flex items-center gap-3 text-white/80 text-sm",
  brandFeatureIcon: "w-5 h-5 text-teal-300 flex-shrink-0",
  
  // Right panel - form card
  cardContainer: "w-full",
  formCard: "bg-white/95 backdrop-blur-xl p-6 sm:p-8 md:p-10 flex flex-col justify-center min-h-[520px]",
  
  // Header (inside form card - compact on signup)
  header: "relative mb-8",
  backButton: "absolute -top-1 -left-1 p-2 text-slate-500 hover:text-slate-700 hover:bg-slate-100 rounded-xl transition-colors",
  headerTitle: "text-2xl md:text-3xl font-bold text-slate-800 tracking-tight",
  headerSubtitle: "text-slate-500 mt-1.5 text-sm",
  avatar: "w-14 h-14 rounded-2xl bg-gradient-to-br from-teal-500 to-cyan-600 flex items-center justify-center mb-4 shadow-lg",
  
  // Form
  formContainer: "space-y-5",
  apiError: "p-3 rounded-xl bg-red-50 text-red-600 text-sm flex items-center gap-2",
  fieldError: "mt-1.5 text-sm text-red-600",
  label: "block text-sm font-semibold text-slate-700 mb-1.5",
  inputContainer: "relative",
  inputIcon: "absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-400",
  input: "w-full pl-11 pr-4 py-3.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-teal-500/30 focus:border-teal-500 focus:bg-white transition-all outline-none",
  passwordInput: "w-full pl-11 pr-12 py-3.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-teal-500/30 focus:border-teal-500 focus:bg-white transition-all outline-none",
  passwordToggle: "absolute inset-y-0 right-0 pr-4 flex items-center text-slate-400 hover:text-slate-600",
  checkboxContainer: "flex items-center gap-3",
  checkbox: "w-4 h-4 text-teal-600 border-slate-300 rounded focus:ring-teal-500",
  checkboxLabel: "text-sm text-slate-600",
  button: "w-full bg-gradient-to-r from-teal-500 to-cyan-600 hover:from-teal-600 hover:to-cyan-700 text-white py-3.5 rounded-xl font-semibold shadow-lg hover:shadow-xl transition-all flex items-center justify-center",
  buttonDisabled: "opacity-70 cursor-not-allowed",
  signInContainer: "mt-6 pt-6 border-t border-slate-200 text-center",
  signInText: "text-slate-600 text-sm",
  signInLink: "font-semibold text-teal-600 hover:text-teal-700 hover:underline",
  spinner: "animate-spin -ml-1 mr-2 h-5 w-5 text-white"
};
...
export const styles = {
  ...
  container: "space-y-6 max-w-7xl mx-auto p-4",
  headerCard: "bg-white rounded-2xl p-6 lg:p-8 -mx-3 shadow-sm border border-gray-100 mb-8",
  title: "text-2xl md:text-3xl font-bold text-gray-800",
  subtitle: "text-gray-500 mt-1",
  grid: "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6",
  budgetCard: "bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-md transition-all",
  cardHeader: "flex justify-between items-center mb-4",
  categoryName: "font-bold text-gray-700 text-lg",
  amountInfo: "text-sm text-gray-500",
  progressBarContainer: "w-full h-3 bg-gray-100 rounded-full overflow-hidden mb-2",
  progressBar: (percent, color) => `h-full rounded-full transition-all duration-500 ${color}`,
  statText: "flex justify-between text-sm font-medium mt-2",
  remainingText: (isOver) => isOver ? "text-red-500" : "text-teal-600",
  inputGroup: "space-y-2",
  input: "w-full border border-gray-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all",
  saveButton: "bg-teal-500 hover:bg-teal-600 text-white px-6 py-3 rounded-xl font-bold transition-all shadow-md hover:shadow-lg w-full mt-4",
};

export const goalStyles = {
  container: "space-y-6 max-w-7xl mx-auto p-4",
  headerCard: "bg-gradient-to-br from-indigo-500 to-purple-600 rounded-2xl p-8 text-white shadow-xl mb-8 relative overflow-hidden",
  headerOverlay: "absolute top-0 right-0 w-64 h-64 bg-white/10 rounded-full -mr-20 -mt-20 blur-3xl",
  title: "text-3xl font-bold mb-2",
  subtitle: "text-indigo-100",
  grid: "grid grid-cols-1 lg:grid-cols-2 gap-6",
  goalCard: "bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-md transition-all relative",
  progressCircleContainer: "relative w-24 h-24 flex items-center justify-center",
  goalInfo: "flex-1",
  goalName: "text-xl font-bold text-gray-800 mb-1",
  targetDate: "text-sm text-gray-500 flex items-center gap-1 mb-4",
  amountStatus: "flex justify-between items-end mb-2",
  currentAmount: "text-2xl font-bold text-indigo-600",
  targetAmount: "text-sm text-gray-400",
  tag: "px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider",
  activeTag: "bg-indigo-100 text-indigo-600",
  completedTag: "bg-green-100 text-green-600",
  addButton: "fixed bottom-8 right-8 bg-indigo-600 hover:bg-indigo-700 text-white w-14 h-14 rounded-full shadow-2xl flex items-center justify-center transition-all hover:scale-110 z-40",
};
